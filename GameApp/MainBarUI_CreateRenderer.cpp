...
{
	{
		...
		ExpBarUI_->OriginPosition_ = GetTransform()->GetLocalPosition() + float4(0.0f, 34.0f);
	}

	{
		...
		Renderer->SetUILocalPosition(ExpBarUI_->OriginPosition_, { 0.0f, -24.0f });
	}

	{
		WzUIRenderer* Renderer = CreateTransformComponent<WzUIRenderer>(GetTransform());
		Renderer->SetImage("mainBar.EXPBar.1024.layer_cover.png");
		Renderer->SetUILocalPosition(ExpBarUI_->OriginPosition_, { -116.0f, -25.0f });
	}

	{
		StatusUI_ = CreateTransformComponent<WzUIRenderer>(GetTransform(), 1);
		StatusUI_->SetImage("mainBar.status.layer_cover.png");
		StatusUI_->SetLocalPosition({ 5.0f, 45.0f - GameEngineWindow::GetInst().GetSize().hy() });
...

	{
		...
		Renderer->SetUILocalPosition(CashShopUI_->OriginPosition_, { -34.0f, 0.0f });
	}

	{
		WzUIRenderer* Renderer = CreateTransformComponent<WzUIRenderer>(GetTransform());
		Renderer->SetImage("menu.button_Character.mouseOver.0.png");
		Renderer->SetUILocalPosition(CashShopUI_->OriginPosition_, { -69.0f, 0.0f });
	}

	{
		WzUIRenderer* Renderer = CreateTransformComponent<WzUIRenderer>(GetTransform());
		Renderer->SetImage("menu.button_Community.disabled.0.png");
		Renderer->SetUILocalPosition(CashShopUI_->OriginPosition_, { -104.0f, 0.0f });
	}

	{
		WzUIRenderer* Renderer = CreateTransformComponent<WzUIRenderer>(GetTransform());
		Renderer->SetImage("menu.button_Setting.normal.0.png");
		Renderer->SetUILocalPosition(CashShopUI_->OriginPosition_, { -139.0f, 0.0f });
	}

	{
		WzUIRenderer* Renderer = CreateTransformComponent<WzUIRenderer>(GetTransform());
		Renderer->SetImage("menu.button_Menu.pressed.0.png");
		Renderer->SetUILocalPosition(CashShopUI_->OriginPosition_, { -174.0f, 0.0f });
	}
}