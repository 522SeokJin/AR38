...
GameEngineBlend* GameEngineBlendManager::Create(const std::string& _Name,
	const D3D11_BLEND_DESC& _Info, float4 _Factor, UINT _Mask)
{
	...
	NewRes->Create(_Info, _Factor, _Mask);
...
GameEngineBlend* GameEngineBlendManager::CreateAndFind(const std::string& _Name, const D3D11_BLEND_DESC& _Info, float4 _Factor, UINT _Mask)
{
	GameEngineBlend* FindRes = Find(_Name);
