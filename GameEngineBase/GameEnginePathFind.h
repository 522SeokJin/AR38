...
public:
	void CalLen(PathIndex _EndIndex)
	{
		...
		{
			float4 ParentIndex = { static_cast<float>(Parent->Index.X), 
				static_cast<float>(Parent->Index.Y) };
			float4 ThisIndex = { static_cast<float>(Index.X), 
				static_cast<float>(Index.Y) };
			PathLen = Parent->PathLen + (ParentIndex - ThisIndex).Len2D();
		}

		float4 EndIndex = { static_cast<float>(_EndIndex.X),
			static_cast<float>(_EndIndex.Y) };
		float4 ThisIndex = { static_cast<float>(Index.X), 
			static_cast<float>(Index.Y) };
...
public:
	...
	// 대각선 안탐색
	std::list<PathIndex> AStarFind4Way(PathIndex Start, PathIndex End, 
		std::function<bool(PathIndex)> _OpenFunction, int _Maximum = 1000);

	// FindTile(0, 2);

	// 대각선도 탐색
	std::list<PathIndex> AStarFind8Way(PathIndex Start, PathIndex End, 
		std::function<bool(PathIndex)> _OpenFunction, int _Maximum = 1000);
