...
public:
	...
	// 쓰레드 안에서 실행은 시켜줘야 한다.
	BOOL WaitforWork(DWORD& lpNumberOfBytesTransferred, ULONG_PTR& lpCompletionKey, 
		LPOVERLAPPED& lpOverlapped, DWORD dwMilliseconds = INFINITE)
	{
		...
		return GetQueuedCompletionStatus(IocpHandle_, &lpNumberOfBytesTransferred, 
	...
	{
		return PostQueuedCompletionStatus(IocpHandle_, _WorkParameter, 
			reinterpret_cast<ULONG_PTR>(_Ptr), nullptr);
	}
